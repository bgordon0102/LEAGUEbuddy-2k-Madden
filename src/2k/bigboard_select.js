...
export async function execute(interaction) {
    ...
    console.log('[bigboard_select][2k] values:', interaction.values, 'players:', allPlayers.length, 'files:', boardFiles.length);
    ...
    // Search the full allPlayers list for the selected player (exact, then startsWith)
    ...
    if (!selected) {
        console.warn('[bigboard_select][2k] Player not found. normalizedSelected:', normalizedSelected, 'sample:', allPlayers.slice(0, 5).map(p => p.name));
    }
    if (!selected) {
        try {
