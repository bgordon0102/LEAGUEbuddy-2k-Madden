...
export const data = new SlashCommandBuilder()
  .setName('2k-activitycheck')
  .setDescription('Post an activity check to the configured 2K channel (staff only).')
...
export async function execute(interaction) {
  ...
  if (!activityChannelId) {
    await interaction.editReply({ content: 'Activity Checks channel not configured for 2K.' });
    return;
  }
