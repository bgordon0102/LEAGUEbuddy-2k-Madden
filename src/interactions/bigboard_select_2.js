...
export async function execute(interaction) {
    ...
    if (!selected) {
        console.warn('[bigboard_select_2] Player not found. normalizedSelected:', normalizedSelected, 'sample:', allPlayers.slice(0, 5).map(p => p.name));
    }
    if (!selected) {
        try {
            await interaction.editReply({ content: "Player not found." });
        } catch (err) {
            console.error('bigboard_select_2: Failed to editReply for not found:', err);
        }
        return;
    }
