const customId = "trade_modal_submit";
...
export async function execute(interaction) {
    ...
    console.log('[DEBUG] trade_modal_submit handler called', { customId: interaction.customId, user: interaction.user?.id, interactionId: interaction.id });
    let responded = false;
    if (!canTrade()) {
        const state = getSeasonState();
        await interaction.reply({ content: `Trades are open Weeks 1-15 and in the offseason. Locked Weeks 16-29 and during playoffs. Current week: ${state.currentWeek}, phase: ${state.phase}.`, flags: 64 });
        return;
    }
    ...
    try {
        ...
        // Resolve coachB by normalizing team labels and supporting "Coach" suffix and aliases
        ...
        const inputRaw = otherTeam || '';
        const inputNoCoach = inputRaw.replace(/coach$/i, '').trim();
        const aliasHit = aliases[norm(inputNoCoach)] || inputNoCoach;
        const candidates = [
            inputRaw,
            inputNoCoach,
            `${inputNoCoach} Coach`,
            aliasHit,
            `${aliasHit} Coach`,
            resolveTeamNameForRoster(inputNoCoach),
            `${resolveTeamNameForRoster(inputNoCoach)} Coach`,
        ].filter(Boolean);

        let coachBId = null;
        for (const cand of candidates) {
            if (coachMap[cand]) { coachBId = coachMap[cand]; break; }
        }
        if (!coachBId) {
            const targetNorm = norm(inputNoCoach);
            for (const [team, id] of Object.entries(coachMap)) {
                const tn = norm(team);
                if (tn === targetNorm || tn.includes(targetNorm) || targetNorm.includes(tn)) {
                    coachBId = id;
                    console.log(`[trade_modal_submit] Matched '${otherTeam}' -> '${team}' via fuzzy`);
                    break;
                }
            }
        }
...
console.log('[DEBUG] trade_modal_submit handler registered:', { customId });
