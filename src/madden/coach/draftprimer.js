...
function prospectRank(prospect) {
    const rank = Number(prospect?.RNK ?? prospect?.rank ?? prospect?.order ?? 9999);
    return Number.isFinite(rank) ? rank : 9999;
}

function projectedRoundForProspect(prospect) {
    const explicit = Number(prospect?.round);
    if (Number.isFinite(explicit) && explicit > 0) return explicit;
    return Math.max(1, Math.floor((prospectRank(prospect) - 1) / 32) + 1);
}

function projectedPickForProspect(prospect) {
    const explicit = Number(prospect?.pick);
    if (Number.isFinite(explicit) && explicit > 0) return explicit;
    return prospectRank(prospect);
}

function formatDevTraitShort(prospect) {
    const tier = Number(prospect?.dev_trait);
...
export async function execute(interaction) {
    ...
    const projectedRound = (p) => projectedRoundForProspect(p);
    ...
    const exposureWeightFor = (player, need, round) => {
        const base = exposureCounts.get(`${player?.name}|${need}|${round}`) || 0;
        const threshold = round === 1 ? 2 : round <= 3 ? 3 : 4;
        return Math.max(0, base - threshold);
    };

    let pickEntries = [];
    const top3Needs = (needs.length ? needs.slice(0, 3) : ['BPA']).filter(Boolean);

    const allLeagueTeams = (league?.teams?.leagueTeamInfoList || []).map((team) => ({
        teamId: Number(team.teamId),
        teamName: getFullTeamName(team, `Team ${team.teamId}`),
    }));
    const naturalSlotByNorm = new Map(
        rawOrder.map((pick, index) => [normalizeName(pick.name || pick.nick || ''), index + 1]),
    );
    for (const leagueTeam of allLeagueTeams) {
        const otherTeamNorm = normalizeName(leagueTeam.teamName);
        const otherNeeds = resolveTeamNeeds(leagueTeam.teamName, league, needsByTeam).slice(0, 3);
        if (!otherNeeds.length) continue;
        const otherRoundOneSlots = tradedOrder
            .map((pick, index) => ({ slot: index + 1, norm: normalizeName(pick.name || pick.nick || '') }))
            .filter((pick) => pick.norm === otherTeamNorm);
        const otherNaturalSlot = Math.max(1, naturalSlotByNorm.get(otherTeamNorm) || 16);
        for (let round = 1; round <= 3; round++) {
            const otherBasePick = round === 1
            ...
            for (const need of otherNeeds) {
                const candidates = otherWindow
                    .filter((player) => need === 'BPA' || mapNeed(player) === need)
                    .sort((a, b) => {
                        const aDistance = Math.abs(prospectRank(a) - otherBasePick);
                        const bDistance = Math.abs(prospectRank(b) - otherBasePick);
                        if (aDistance !== bDistance) return aDistance - bDistance;
                        return prospectRank(a) - prospectRank(b);
                    })
                    .slice(0, 3);
                for (const candidate of candidates) {
                    const key = `${candidate.name}|${need}|${round}`;
    ...
    for (let round = 1; round <= 7; round++) {
        ...
        const rankedCandidatesForNeed = (pool, need) => {
            const filtered = pool.filter((player) => {
                ...
                const mappedNeed = mapNeed(player);
                if (need === 'BPA') return true;
                return mappedNeed === need;
            });
            return filtered.sort((a, b) => {
                const aExposure = exposureWeightFor(a, need, round);
                ...
                const aScoutScore =
                    ...
                    (aScout.isPartiallyScouted ? -2 : 0) +
                    (aScout.hasBoardHistory ? -1 : 0);
                const bScoutScore =
                    (bScout.isSuggestedScout ? -4 : 0) +
                    (bScout.isFullyScouted ? -3 : 0) +
        ...
        const chooseDiversifiedCandidate = (pool, need) => {
            const ranked = rankedCandidatesForNeed(pool, need);
        ...
        const pickForNeeds = (pool, needsList) => {
            for (const need of needsList) {
                const hit = chooseDiversifiedCandidate(pool, need);
                if (hit) return hit;
            }
            return null;
        };
        ...
        pickEntries.push(target
            ? {
                ...
                target,
                need: targetNeed,
                line: `${round}. ${target.name} (${target.position || target.position_1 || 'POS'}) — ${school}`,
            }
    ...
    if (process.env.MOCK_DEBUG) console.log('[WR CAP]', teamName, 'wrIndices', wrIndices.length, 'pool', replacementPool.length);
        ...
        for (let i = 2; i < wrIndices.length; i++) {
            ...
            pickEntries[targetIdx] = {
                ...
                target: rep,
                need: repNeed,
                line: `${targetIdx + 1}. ${rep.name} (${rep.position || rep.position_1 || 'POS'}) — ${school}`,
            };
        }
    }
    ...
    const boardNeedOrder = pickEntries
        .map((entry) => entry?.need)
        .filter(Boolean)
        .filter((need, idx, arr) => arr.indexOf(need) === idx)
        .slice(0, 3);
    const boardShapeLine = boardNeedOrder.length >= 3
        ? `Board shape: stay anchored on ${formatNeedLabel(boardNeedOrder[0]).toLowerCase()} early, then let value carry you toward ${formatNeedLabel(boardNeedOrder[1]).toLowerCase()} and ${formatNeedLabel(boardNeedOrder[2]).toLowerCase()}.`
        : boardNeedOrder.length === 2
            ? `Board shape: open with ${formatNeedLabel(boardNeedOrder[0]).toLowerCase()} and keep ${formatNeedLabel(boardNeedOrder[1]).toLowerCase()} as the clean fallback when the tier shifts.`
            : boardNeedOrder.length === 1
                ? `Board shape: this class lines up most cleanly with ${formatNeedLabel(boardNeedOrder[0]).toLowerCase()}, so do not drift off that lane without clear surplus value.`
                : '';
    const archetypeLine = (() => {
        const prospect = recommendedProspectForNeed(primaryNeed);
    ...
    const competitionForTarget = (entry) => {
        ...
        const teamsAhead = tradedOrder
            ...
            .map((pick) => {
                ...
                const pickNeeds = resolveTeamNeeds(pickTeam, league, needsByTeam).slice(0, 3);
                return { pickTeam, pickNeeds };
            })
            .filter((pick) => normalizeName(pick.pickTeam) !== normalizeName(teamName));
        const directCompetition = teamsAhead.filter((pick) => pick.pickNeeds.includes(targetNeed));
        const pressure = directCompetition.length >= 5
            ? 'hot'
        ...
        pickEntries = pickEntries.map((entry, idx) => {
            ...
            if (replacement) {
                ...
                return {
                    ...
                    line: `${idx + 1}. ${replacement.name} (${replacement.position || replacement.position_1 || 'POS'}) — ${school}`,
                };
            }
    ...
    const marketIntel = topBoardEntries.map((entry, idx) => {
        ...
        if (!intel) return `${idx + 1}. ${entry.target.name}: no market read available yet.`;
        const target = entry.target;
        ...
        const competitionTeams = intel.directCompetition
            .slice(0, 2)
            .map((pick) => pick.pickTeam)
            .join(', ');
        const needLabel = formatNeedLabel(intel.targetNeed).toLowerCase();
        const fallbackNames = topBoardEntries
            .filter((other) => other?.target?.name && other.target.name !== target.name && (other.need || mapNeed(other.target)) === intel.targetNeed)
    ...
    const planNames = topBoardEntries
        ...
        .filter(Boolean)
        .slice(0, 2);
    const phaseShort = seasonContext.phase === 'early_regular'
        ? 'Early board build'
        : seasonContext.phase === 'mid_regular'
            ? 'Midseason board build'
            : seasonContext.phase === 'late_regular'
                ? 'Stretch-run board build'
                : seasonContext.phase === 'postseason'
                    ? 'Final board prep'
                    : 'Offseason board build';
    const capitalShort = roundOneOwnedSlots.length
        ? extraRoundOneSlots.length
            ? `With picks #${firstRoundSlot} and ${extraRoundOneSlots.map((pick) => `#${pick.slot}`).join(', ')}, lock one premium need and keep the other slot flexible.`
            : `At pick #${firstRoundSlot}, stay in range and avoid chasing names that will not reach your slot.`
        : 'Without a first-rounder, build a deeper day-two cluster and stay patient.';
    const fallbackShort = secondaryNeed && secondaryNeed !== primaryNeed
        ? `Start at ${formatNeedLabel(primaryNeed).toLowerCase()}, with ${formatNeedLabel(secondaryNeed).toLowerCase()} as the fallback lane.`
        : `Stay anchored on ${formatNeedLabel(primaryNeed).toLowerCase()} and let value guide the next tier.`;
    const namesShort = planNames.length === 2
        ? `Current names to hold near the top: ${planNames[0]} and ${planNames[1]}.`
        : planNames.length === 1
            ? `Current top name: ${planNames[0]}.`
            : '';
    const inSeasonShort = seasonContext.isInSeason
    ...
    const quickCapital = roundOneOwnedSlots.length
        ? extraRoundOneSlots.length
            ? `1st-round picks: #${firstRoundSlot} and ${extraRoundOneSlots.map((pick) => `#${pick.slot}`).join(', ')}.`
            : `1st-round pick: #${firstRoundSlot}.`
        : 'No 1st-round pick.';
    const quickPriority = needsDisplay.slice(0, 3).map((need, idx) => `${idx + 1}. ${formatNeedLabel(need)}`).join(' | ');
    const draftPlan = [phaseShort, capitalShort, fallbackShort, namesShort, inSeasonShort]
        .filter(Boolean)
        .join(' ')
        .replace(/\s+/g, ' ')
        .trim();
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(seasonContext.isInSeason
            ? `Pre-draft board built from your top needs, live season context, and ${draftYear} draft capital. Use it to decide who to scout, where your tiers start, and when you may need a pivot or trade-up.`
            : `Pre-draft board built from your top needs, class shape, and ${draftYear} draft capital.`)
        .addFields(
            { name: 'Team Snapshot', value: quickSnapshot, inline: false },
