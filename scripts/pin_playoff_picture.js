...
async function main() {
  ...
  client.once('ready', async () => {
    console.log(`Logged in as ${client.user.tag}, updating Playoff Picture for league ${leagueId}...`);
    try {
      await updatePlayoffPicture(client, leagueId);
      console.log('Playoff Picture embed updated.');
    } catch (e) {
      console.error('Failed to update Playoff Picture:', e?.message || e);
    } finally {
      client.destroy();
    }
  });
